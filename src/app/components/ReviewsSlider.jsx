...
const ReviewsSlider = () => {
  return (
    <>
        <Marquee gradient={false} speed={50} pauseOnHover={true} className="flex items-center justify-start w-full mb-4">
            {reviews.slice(0, Math.floor((reviews.length-1)/2)).map((review, index) => (
                <ReviewsCard key={index} info={review}/>
            ))}
        </Marquee>
        <Marquee gradient={false} direction={"right"} speed={50} pauseOnHover={true} className="flex items-center justify-start w-full">
